...
int atMostSum(vector<int> &arr, int k)
{
    ...
    for (int l = 0, r = 0; r <= n - 1; r++) // we're counting number of substrings and not finding the longest substring hence we must use while loop
    {
        sum += arr[r];
        while (sum > k)
        {
...
// for counting nice subarrays add % 2 in  sum += arr[r] and sum -= arr[l] as sum += arr[r]%2 and sum -= arr[l]%2
